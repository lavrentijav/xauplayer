...
object AppConfig {
    ...
    /**
     * Насколько близко к длительности главы позиция считается «конец главы».
     * Нужно, чтобы обрыв на последних байтах не мешал автопереходу на следующую главу.
     */
    const val END_OF_CHAPTER_THRESHOLD_MS = 3000L

    /**
     * Сколько ждать, если плеер завис в буферизации на конце главы,
     * прежде чем считать главу доигранной и переходить к следующей.
     */
    const val END_OF_CHAPTER_STALL_TIMEOUT_MS = 12000L

    /**
     * Максимум попыток пересобрать поток после ошибки загрузки.
     * Каждая попытка заново запрашивает URL главы: подписанные ссылки хранилища
     * протухают, и повтор по старому URL бессмысленен.
     */
    const val STREAM_RETRY_MAX_ATTEMPTS = 5
    ...
    /**
     * Максимальное количество попыток retry внутри ExoPlayer.
     * Не бесконечность: при бесконечных повторах плеер навсегда остаётся в
     * STATE_BUFFERING, не отдаёт STATE_ENDED и не сообщает об ошибке — глава
     * «зависает» на конце и автопереход на следующую не срабатывает.
     * Исчерпав их, плеер отдаёт ошибку, а приложение переоткрывает поток само
     * (см. STREAM_RETRY_MAX_ATTEMPTS).
     */
